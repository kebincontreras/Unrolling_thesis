...
from project.model.unrolling_jorge_physics import JorgePhysicsADMMUnrollingNet, apply_filter
...
def evaluate(
    model: JorgePhysicsADMMUnrollingNet,
...
def build_final_report(
    ...
    rows = [
        ...
        ("Ours (Jorge-Physics)", summarize_metric(ours_ssim), summarize_metric(ours_psnr), summarize_metric(ours_lpips)),
    ]
    ...
    report_txt = [
        "Final metrics",
        "Model = Jorge-ADMM-Physics-Unrolling",
...
def run_experiment_jorge_physics(config: ExperimentConfig) -> None:
    ...
    model = JorgePhysicsADMMUnrollingNet(config, psf_tensor).to(device)
